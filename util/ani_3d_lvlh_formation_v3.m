function ani_3d_lvlh_formation_v3(sensor_data, chief_mesh, camera_params, ...
                                   LVLH_pos, euler_angles, colors, r_init_B, t1)
% V2: 카메라 픽셀 뷰에 오프스크린 렌더링 지구 표시 + OBJ 모델 사용
%
% 수정된 입력:
%   chief_mesh - OBJ 메쉬 구조체 (vertices, faces)

if ~exist('chief_mesh', 'var') || isempty(chief_mesh) || ~isfield(sensor_data, 'z_measured')
    warning('필요한 데이터가 없어 애니메이션을 실행할 수 없습니다.');
    return;
end

fig = figure('Name', 'Integrated Sensor Model with Earth', 'Position', [50, 50, 1900, 950], 'Color', 'k');
fprintf('통합 애니메이션 v2 시작 (OBJ 모델 + 오프스크린 렌더링 지구 표시)...\n');

skip_frame = 10;
fov_camera = camera_params.intrinsic.fov;
fov_length = r_init_B * 0.8;
num_points = size(chief_mesh.vertices, 1);  % 버텍스 개수
...
for k = 1:skip_frame:length(t1)
    ...
    % OBJ 메쉬 버텍스를 LVLH 프레임으로 변환
    vertices_body = chief_mesh.vertices;  % Nx3
    vertices_lvlh = (R_L2A * vertices_body')';  % Nx3
    ...
    % Chief OBJ 모델 렌더링
    patch('Vertices', vertices_lvlh, ...
          'Faces', chief_mesh.faces, ...
          'FaceColor', colors.points, ...
          'EdgeColor', 'none', ...
