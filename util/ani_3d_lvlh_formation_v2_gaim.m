function ani_3d_lvlh_formation_v2_gaim(sensor_data, initial_pointcloud, camera_params, ...
...
for k = 1:skip_frame:length(t1)
    ...
    if isfield(sensor_data(k), 'earth_image') && ~isempty(sensor_data(k).earth_image)
        ...
        if ~isempty(visible_idx)
            ...
            plot(pixels_true(1, visible_idx), pixels_true(2, visible_idx), '.', 'Color', colors.meas_proj, 'MarkerSize', 0.1);
        end
