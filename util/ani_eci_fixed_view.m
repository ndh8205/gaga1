function ani_eci_fixed_view(r_A_I, r_B_I, v_A_I, sensor_data, colors, R_e, t1, ani_dt)
% ECI 애니메이션 - Chief 추적 뷰 (OpenGL 최적화)
...
%% ========== Figure 생성 ==========
fig = figure('Name', 'ECI Animation - Chief Tracking', ...
...
grid on;
axis equal;
title('ECI Frame - Chief Tracking with Body Axes', 'FontSize', 12, 'FontWeight', 'bold', 'Color', 'w');
...
for k = 1:skip_frame:length(t1)
    ...
    v_chief = v_A_I(:,k_end);
    v_hat = v_chief / norm(v_chief);
    h_vec = cross(r_chief, v_chief);
    h_hat = h_vec / norm(h_vec);
    
    % 카메라 위치
    cam_pos = r_chief + r_hat * 0.8;
    ...
    % 타이틀
    set(h_title, 'String', sprintf('t = %.1f s | Alt: %.1f km', ...
                 t1(k_end), norm(r_chief) - R_e));
