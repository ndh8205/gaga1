function ani_3d_lvlh_formation_v4_gaim(sensor_data, initial_pointcloud, camera_params, ...
...
% 기존 V2와 동일한 Figure 설정
fig = figure('Name', 'Integrated Sensor Model V3', 'Position', [50, 50, 1900, 950], 'Color', 'k');
fprintf('통합 애니메이션 v3 시작...\n');
...
for k = 1:skip_frame:length(t1)
    ...
    % Face 렌더링 체크
    if isstruct(z_true) && isfield(z_true, 'face_info') && ~isempty(z_true.face_info)
        % Face 모드
        face_info = z_true.face_info;
        
        % Depth order로 faces 그리기
        for i = 1:length(face_info.depth_order)
            f = face_info.depth_order(i);
            if face_info.visible(f)
                idx = face_info.faces(f, :);
                
                % 유효한 인덱스인지 체크
                if all(idx <= num_points)
                    tri_x = pixels_true(1, idx([1 2 3 1]));
                    tri_y = pixels_true(2, idx([1 2 3 1]));
                    
                    % Wireframe으로 그리기
                    plot(tri_x, tri_y, 'w-', 'LineWidth', 0.5);
                    
                    % 또는 채워진 삼각형 (선택)
                    % fill(tri_x(1:3), tri_y(1:3), [0.7 0.7 0.8], ...
        ...
        if ~isempty(visible_idx)
            plot(pixels_true(1, visible_idx), pixels_true(2, visible_idx), ...
                 '.', 'Color', colors.meas_proj, 'MarkerSize', 0.1);
        end
    end
    ...
    % Face 모드인지 Point 모드인지에 따른 제목
    if isstruct(z_true) && isfield(z_true, 'face_info')
        title(sprintf('Camera View (Face Mode)\nVisible: %d faces', ...
              sum(z_true.face_info.visible)), 'Color', 'w');
    else
        title(sprintf('Camera View (Point Mode)\nVisible: %d pts', ...
              sum(visible_measured > 0.5)), 'Color', 'w');
    end
